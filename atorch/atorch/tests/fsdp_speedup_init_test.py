...
def run_module_gt(rank, world_size, hidden_size):
    ...
    loss.backward()
    optim.step()
    save_fsdp_flat_param(model, f"/tmp/fsdp_speedup_init_test/{world_size}/gt")
    save_fsdp_optim_param(model, optim, f"/tmp/fsdp_speedup_init_test/{world_size}/gt")


def run_module_reshard(rank, world_size, ckpt_path, hidden_size, test_dir_prefix):
    os.environ["LOCAL_RANK"] = str(rank)
    os.environ["RANK"] = str(rank)
    ...
    save_fsdp_flat_param(model, f"/tmp/fsdp_speedup_init_test/{test_dir_prefix}/reshard/{world_size}")
    save_fsdp_optim_param(model, optim, f"/tmp/fsdp_speedup_init_test/{test_dir_prefix}/reshard/{world_size}")
