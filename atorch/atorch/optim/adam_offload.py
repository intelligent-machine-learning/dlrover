...
class PartitionAdam(Adam):
    ...
    def debug_optimizer(self, debug_optimizer, prefix):
        while hasattr(debug_optimizer, "optimizer"):
            debug_optimizer = debug_optimizer.optimizer
        optimizer_numel = sum([p["numel"] for p in debug_optimizer.state.values() if isinstance(p, dict)])
        optimizer_params = [p["state_block"] for p in debug_optimizer.state.values() if isinstance(p, dict)]
        state_length = len(debug_optimizer.state)
        params_numel = (
            [p.numel() for p in optimizer_params]
            + [len(debug_optimizer.param_groups)]
            + [p.numel() for i, group in enumerate(debug_optimizer.param_groups) for p in group["params"]]
        )
        if optimizer_params:
            optimizer_mean = torch.mean(torch.cat(optimizer_params))
            optimizer_std = torch.std(torch.cat(optimizer_params))
        else:
            optimizer_mean = 0
            optimizer_std = 0
        print(
            f"state dict of optimizer rank {torch.distributed.get_rank()} {prefix} state={state_length}"
            f"total_params={optimizer_numel} list:{params_numel} mean={optimizer_mean}, std={optimizer_std}"
        )
    ...
    def state_dict(self):
        ...
        if self.inner_optimizers:
            ...
            #     self.debug_optimizer(optimizer, "%s-%s" % (idx, "dump"))
            # self.debug_optimizer(self, "root-dump")
        return one_dict
    ...
    def load_state_dict(self, state_dict):
        ...
        if self.inner_optimizers and "inner_optimizers" in state_dict:
            ...
            #         self.inner_optimizers[key], "%s-%s" % (key, "load")
            #     )
            # self.debug_optimizer(self, "root-load")
