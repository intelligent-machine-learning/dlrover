...
class DesignSpace:
    ...
    def parse(self, rec):
        ...
        assert len(self.para_names) == len(set(self.para_names))
        return self
    ...
    def transform(self, data):
        ...
        return xc, xe.astype(int)

    def inverse_transform(self, x: np.ndarray, xe: np.ndarray) -> pd.DataFrame:
        """
        input: x and xe
        ...
        inv_dict = {}
        for i, name in enumerate(self.numeric_names):
            inv_dict[name] = self.paras[name].inverse_transform(x[:, i])
        for i, name in enumerate(self.enum_names):
            inv_dict[name] = self.paras[name].inverse_transform(xe[:, i])
