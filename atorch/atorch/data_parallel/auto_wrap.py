...
def default_auto_wrap_policy(
    ...
    Args:
       ...
       exclude_wrap_modules (Set[Type[nn.Module]]):atorch/data_parallel/auto_wrap.py
...
class ConfigAutoWrap:
    ...
    def __init__(self, auto_wrap_policy: Optional[Callable] = None, **kwargs: Dict[str, Any]) -> None:
    ...
    def recursive_wrap(module: nn.Module, auto_wrap_policy: Optional[Callable], **kwargs: Any) -> Tuple[nn.Module, int]:
        ...
        # Make sure no child is not already wrapped. # module reuse will cause assert fail
        for _, child in module.named_modules():
            if isinstance(child, cast(type, ConfigAutoWrap.wrapper_cls)):
                return module, num_params

        assert auto_wrap_policy is not None
        first_policy_result = auto_wrap_policy(module=module, recurse=True, unwrapped_params=num_params)
        if first_policy_result:
            total_wrapped_params = 0
            # Iterate through the children, recursively wrap if necessary
            if first_policy_result != "wrap_skip_child":
                for name, child in module.named_children():
                    (
                        wrapped_child,
                        num_wrapped_params,
                    ) = ConfigAutoWrap.recursive_wrap(module=child, auto_wrap_policy=auto_wrap_policy, **kwargs)
            ...
            after_child_policy_result = auto_wrap_policy(module=module, recurse=False, unwrapped_params=remainder)
            if after_child_policy_result:
