 ...
 */
#ifndef ATORCH_NPU_CSRC_INC_PYTORCH_NPU_HELPER_H_
#define ATORCH_NPU_CSRC_INC_PYTORCH_NPU_HELPER_H_
...
inline c10::Scalar ConvertTensorToScalar(const at::Tensor& tensor) {
  ...
  if (aclInput->scalar_type() == at::ScalarType::Double) {
    double value = *reinterpret_cast<double*>(aclInput->data_ptr());
    c10::Scalar scalar(value);
    expScalar = scalar;
  } else if (aclInput->scalar_type() == at::ScalarType::Long) {
    int64_t value = *reinterpret_cast<int64_t*>(aclInput->data_ptr());
    c10::Scalar scalar(value);
    expScalar = scalar;
  } else if (aclInput->scalar_type() == at::ScalarType::Float) {
    float value = *reinterpret_cast<float*>(aclInput->data_ptr());
    c10::Scalar scalar(value);
    expScalar = scalar;
  } else if (aclInput->scalar_type() == at::ScalarType::Int) {
    int value = *reinterpret_cast<int*>(aclInput->data_ptr());
    c10::Scalar scalar(value);
    expScalar = scalar;
    ...
    int8_t value = *reinterpret_cast<int8_t*>(aclInput->data_ptr());
    c10::Scalar scalar(value);
    expScalar = scalar;
...
inline at::Tensor CopyTensorHostToDevice(const at::Tensor& cpu_tensor) {
  ...
  return cpuPinMemTensor.to(
      c10::Device(torch_npu::utils::get_npu_device_type(), deviceIndex),
...
#endif  // ATORCH_NPU_CSRC_INC_PYTORCH_NPU_HELPER_H_
