...
from .abstract_accelerator import BaseAccelerator
...
class CUDA_Accelerator(BaseAccelerator):
    def __init__(self):
        self._name = "cuda"
    ...
    def op_builder_dir(self):
        try:
            # is op_builder from atorch or a 3p version? this should only succeed if it's atorch
            # if successful this also means we're doing a local install and not JIT compile path
            from op_builder import __atorch__  # noqa: F401 # type: ignore

            return "op_builder"
        except ImportError:
            return "atorch.ops.op_builder"
