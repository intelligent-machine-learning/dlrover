...
template <typename T, int numBits, dequantize::Type qType, int unroll,
          int threads>
__global__ void dequantize_kernel(T* __restrict__ dequant_data,
                                  ...
                                  int elems_per_group, int total_elems) {
...
#define LAUNCH_DEQUANT_KERNEL(num_bits, q_type)                    \
  dequantize_kernel<T, num_bits, q_type, unroll, threads>          \
      <<<grid, block, 0, stream>>>(dequant_data, q_data, q_params, \
                                   elems_per_group, total_elems);
...
void launch_dequantize_kernel(T* dequant_data, const int8_t* q_data,
  ...
  constexpr int elems_per_block =
      unroll * threads * dequantize::granularity / (sizeof(T));

  const dim3 block(threads);
  const dim3 grid((total_elems + elems_per_block - 1) / elems_per_block);

  // TODO(cmikeh2): It may make sense to tune unroll, there is perf benefit for
  // large problem sizes with this large unroll value.
