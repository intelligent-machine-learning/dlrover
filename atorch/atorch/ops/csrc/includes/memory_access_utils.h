...
template <int AccessSize>
__device__ __forceinline__ void memcpy_async_nop(void* shr, const void* gbl,
                                                 bool predicate);

template <int AccessSize>
__device__ __forceinline__ void memcpy_async_zero(void* shr, const void* gbl,
                                                  bool predicate);
...
template <>
__device__ __forceinline__ void load_global<16, LoadPolicy::CacheGlobal>(
...
template <>
__device__ __forceinline__ void load_global<16, LoadPolicy::CacheGlobal>(
    void* dst, const void* src, bool do_access) {
  uint4* data = reinterpret_cast<uint4*>(dst);
#ifdef PTX_AVAILABLE
  asm volatile(
      "{\n"
...
template <>
__device__ __forceinline__ void load_global<16, LoadPolicy::CacheStreaming>(
    void* dst, const void* src) {
  uint4* data = reinterpret_cast<uint4*>(dst);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void load_global<8, LoadPolicy::CacheGlobal>(
    void* dst, const void* src) {
  uint2* data = reinterpret_cast<uint2*>(dst);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void load_global<8, LoadPolicy::CacheStreaming>(
    void* dst, const void* src) {
  uint2* data = reinterpret_cast<uint2*>(dst);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void load_global<4, LoadPolicy::CacheGlobal>(
    void* dst, const void* src) {
  int32_t* data = reinterpret_cast<int32_t*>(dst);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void load_global<4, LoadPolicy::CacheStreaming>(
    void* dst, const void* src) {
  int32_t* data = reinterpret_cast<int32_t*>(dst);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void load_global<2, LoadPolicy::CacheGlobal>(
    void* dst, const void* src) {
  int16_t* data = reinterpret_cast<int16_t*>(dst);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void load_global<2, LoadPolicy::CacheStreaming>(
    void* dst, const void* src) {
  int16_t* data = reinterpret_cast<int16_t*>(dst);
#ifdef PTX_AVAILABLE
...
__device__ __forceinline__ void store_global<16>(void* dst, const void* src) {
  ...
  asm volatile("st.global.wb.v4.u32 [%0], {%1, %2, %3, %4};\n"
               :
               : "l"(dst), "r"(data[0].x), "r"(data[0].y), "r"(data[0].z),
                 "r"(data[0].w)
               : "memory");
#else
...
template <>
__device__ __forceinline__ void store_global<16, StorePolicy::CacheGlobal>(
    void* dst, const void* src) {
  const uint4* data = reinterpret_cast<const uint4*>(src);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void store_global<16, StorePolicy::CacheStreaming>(
    void* dst, const void* src) {
  const uint4* data = reinterpret_cast<const uint4*>(src);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void store_global<8, StorePolicy::CacheGlobal>(
    void* dst, const void* src) {
  const uint2* data = reinterpret_cast<const uint2*>(src);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void store_global<8, StorePolicy::CacheStreaming>(
    void* dst, const void* src) {
  const uint2* data = reinterpret_cast<const uint2*>(src);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void store_global<4, StorePolicy::CacheGlobal>(
    void* dst, const void* src) {
  const int32_t* data = reinterpret_cast<const int32_t*>(src);
#ifdef PTX_AVAILABLE
...
template <>
__device__ __forceinline__ void store_global<4, StorePolicy::CacheStreaming>(
    void* dst, const void* src) {
  const int32_t* data = reinterpret_cast<const int32_t*>(src);
#ifdef PTX_AVAILABLE
...
__device__ __forceinline__ void store_shared<16>(void* dst, const void* src) {
  ...
  asm volatile("st.shared.v4.u32 [%0], {%1, %2, %3, %4};\n"
               :
               : "r"(dst_int), "r"(data[0].x), "r"(data[0].y), "r"(data[0].z),
                 "r"(data[0].w));
#else
  uint4* dst_cast = reinterpret_cast<uint4*>(dst);
  dst_cast[0] = data[0];
...
__device__ __forceinline__ void memcpy_async_zero_nop(void* shr,
  ...
  asm volatile(
      ...
      "}\n"
      :
      : "r"((int)nop_predicate), "r"(shr_int), "l"(gbl), "n"(AccessSize),
        "r"(bytes_to_copy));
}
...
__device__ __forceinline__ void memcpy_async_nop_cg(void* shr, const void* gbl,
                                                    bool predicate) {
  unsigned shr_int = internal::convert_to_shared(shr);
...
__device__ __forceinline__ void memcpy_async_zero_cg(void* shr, const void* gbl,
                                                     bool predicate) {
  unsigned shr_int = internal::convert_to_shared(shr);
...
// TODO: The tail complete should be a known compile time artifact, should try
// and induce this without all of the branches from the call-site. This is a
// hacky solution.
template <>
