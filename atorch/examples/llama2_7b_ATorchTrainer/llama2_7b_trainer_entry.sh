...
if [ ${NODE_RANK} -eq "0" ]; then
    ...
    nvidia-smi >> ${OUTPUT_DIR}/log_llama2_atorch_trainer_${WORLD_SIZE}n${NUM_GPUS}g.txt
    printenv >> ${OUTPUT_DIR}/log_llama2_atorch_trainer_${WORLD_SIZE}n${NUM_GPUS}g.txt
fi
...
python -m atorch.distributed.run --fault_tolerant --max_restarts=0 \
    ...
    --use_bettertransformer_kernels true \
    --enable_torch_profiler false \
    --save_load_by_streaming false \
    --logging_dir ${OUTPUT_DIR} \
    --report_to none \
    2>&1 | tee -a ${OUTPUT_DIR}/log_llama2_atorch_trainer_${WORLD_SIZE}n${NUM_GPUS}g.txt
