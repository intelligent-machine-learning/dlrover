...
class FusedDropPath(nn.Module):
    def __init__(self):
        super().__init__()
        self.drop_path = DropPath(drop_path)

    def forward(self, x, droppath_params1, droppath_params2):
        return x + drop_path(droppath_params1 * droppath_params2)
...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False):
        ...
        if self.q_bias is not None:
            self.qkv_bias = torch.cat((self.q_bias, torch.zeros_like(self.v_bias, requires_grad=False), self.v_bias))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.fused_droppath = FusedDropPath()

    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False):
        # add torch.compile to droppath
        # global compiled
        # if not compiled:
        #     config = CompilerConfig()
        #     npu_backend = tng.get_npu_backend(compiler_config=config)
        #     self.fused_droppath = torch.compile(self.fused_droppath,dynamic=False,fullgraph=True,backend=npu_backend)
        #     compiled = True
        ...
        if self.gamma_1 is None:
            ...
            x = self.fused_droppath(x, self.gamma_1, self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias))
            x = self.fused_droppath(x, self.gamma_2, self.mlp(self.norm2(x)))
        return x
