...
from datasets import build_beit_pretraining_dataset, build_beit_pretraining_coco_pcache_dataset
...
try:
    from torch.distributed.fsdp import FullOptimStateDictConfig  # torch==2.1.0
except ImportError:
    from torch.distributed.fsdp.api import FullOptimStateDictConfig  # torch<2.1.0
...
try:
    torch.npu.config.force_no_jit=True
...
def get_args():
    ...
    # parser.add_argument('--shared_lm_head', action='store_true', help='head_layers')
    ...
    # pcache coco args
    parser.add_argument('--use_pcache', action='store_true',)
    parser.add_argument('--pcache_prefix', default="pcache://", type=str,
                        help='pcache prefix')
    parser.add_argument('--pcache_host', default="pcache-lingjun-sa129-pcacheproxy.pcache.svc.aidc.sa129.alipay.com", type=str,
                        help='pcache host')
    parser.add_argument('--pcache_port', default=39999, type=int,
                        help='pcache port')
    parser.add_argument('--pcache_coco_train_dir', default="/mnt/kepler/coco2017/train2017", type=str,
                        help='train2017 on pcache')
    parser.add_argument('--pcache_coco_val_dir', default="/mnt/kepler/coco2017/val2017", type=str,
                        help='val2017 on pcache')
    parser.add_argument('--coco2017_annotations_dir', default="/workspace/tujuanhui.tjh/multi_turn/cailun.cl/coco2017/annotations/", type=str,
                        help='annotations dir')
    parser.add_argument('--coco2017_split', default="train2017", type=str,
                        help='train or val', choices=["train2017", "val2017"])
    ...
    parser.add_argument('--aprofiler_start_step', default=2, type=int)
    parser.add_argument('--enable_torch_profiler', default=2, type=int)
    parser.add_argument('--enable_aprofiler', action='store_true')
    parser.add_argument('--atorch_init_dist', action='store_true')
    parser.add_argument('--aprofiler_start_step', default=2, type=int)
    parser.add_argument('--aprofiler_end_step', default=10, type=int)
...
def main(args):
    ...
    # get dataset
    if args.use_pcache:
        dataset_train = build_beit_pretraining_coco_pcache_dataset(args)
    else:
