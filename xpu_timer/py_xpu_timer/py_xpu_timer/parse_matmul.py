    ...
    def run(self, path="relative_matmul_performance.pkl", plot=False, p=0.9):
        if plot:
            try:
                ...
                print(
                    "seaborn is not found, run `pip install seaborn` if you need plot"
                )
                return
        ...
        for rank, ops in self.rank_mean_tflops.items():
            relative_performance[rank] = {
                op: ops[op] / self.matmul_database[op]
                if op in self.matmul_database
                else 1.0
                for op in ops
            }
        ...
        comparison_p90 = df.apply(
            lambda row: row < percentile_90[row.name], axis=1
        )
...
def main():
    ...
    parser.add_argument(
        "--path", default="matmul.bin", help="Trace path for diff"
    )
    parser.add_argument(
        "--output-path", default="relative_matmul_performance.pkl"
    )
