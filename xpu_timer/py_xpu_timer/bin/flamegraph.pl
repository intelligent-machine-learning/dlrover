...
sub color {
	...
	if (defined $type and $type eq "python") {
		if ($name =~ /\/torch/) {	# torch
			$type = "green";
		} elsif ($name =~ /nccl/i) {	# nccl
			$type = "green";
		} elsif ($name =~ /::/) {	# C++
			$type = "yellow";
		} elsif ($name =~ /python.*\.c:\d+.*/i) {	# cpython
			$type = "aqua";
		} elsif ($name =~ /python.*\.h:\d+.*/i) {	# cpython
			$type = "aqua";
		} elsif ($name =~ /\.py:\d+.*/) {	# python
			$type = "aqua";
		} elsif ($name =~ /State@/) {	# state
			$type = "purple";
		} else {			# system
			$type = "red";
		}
	} elsif (defined $type and $type eq "tracing_kernel") {
		if ($name =~ /xpu_timer.*_(AllReduce|AllGather|SendRecv|ReduceScatter)/i) {	# torch
			$type = "green";
		} elsif ($name =~ /xpu_timer_.*bmnk/i) {	# nccl
			$type = "red";
		} else {			# system
			$type = "aqua";
		}
