...
class TracingData:
    ...
    @staticmethod
    def parse_one_host_trace_data(packet, rank, host_traces, header_uuid, base_uuid, group_index):
        host = packet.add()
    ...
    def parse_one_trace_data(args):
        path, header_uuid, group_index = args
        name = path.name
        ...
        with path.open("rb") as f:
            ...
            if hasattr(timeline_traces, "host_traces"):
                TracingData.parse_one_host_trace_data(
                    packet, timeline_traces.rank, timeline_traces.host_traces, header_uuid, base_uuid, group_index
                )
        return trace.SerializeToString(), (timeline_traces.rank, matmul_debug_info)

    def __init__(self, path, each_trace, debug_data=None, rank=None):
        ...
        self.delay = each_trace.delay_us

        self.kernel_code = each_trace.kernel_type
        self.start = each_trace.start_us
    ...
    def parse_pb(self, packet, uuid, matmul_debug_info):
        ...
        delay_annotation.uint_value = self.delay
...
def parse_group(group_name, timelines, group_index, add_iternal, fd, concurrency=32):
    ...
    args = [(path, header_uuid, group_index) for path in timelines]
...
def generate_perfetto_trace(args):
    ...
    for index, (name, files) in enumerate(timelines.items()):
        all_matmul_info.MergeFrom(parse_group(name, files, index, add_internel, fd, args.c))
        add_internel = False
