    ...
    def backward(
        ...
        with torch.cuda.nvtx.range("_LayerNormMLP_backward"):
            ...
            if (
                ...
                ln_out_total, _ = gather_along_first_dim(ln_out, ctx.tp_group, async_op=False)
                handle = None
            else:
                ln_out_total = ln_out
                handle = None
            ...
            if ctx.set_parallel_mode and ctx.sequence_parallel:
                ...
                if not ctx.ub_bulk_wgrad and not ctx.ub_overlap_rs_dgrad:
                    ...
                    fc1_dgrad, _ = reduce_scatter_along_first_dim(
                        fc1_dgrad, ctx.tp_group, async_op=False
                    )
                    handle = None
