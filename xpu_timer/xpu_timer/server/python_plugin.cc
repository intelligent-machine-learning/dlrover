...
void xpuTimerDaemonSetUpPython() {
  ...
  if (PyObject_SetAttrString(sys_module, "stdout", devnull) < 0)
    PyErr_Print();

  if (PyObject_SetAttrString(sys_module, "stderr", devnull) < 0)
    PyErr_Print();
...
static PyObject *
convertFrameInfoToDict(const SignalFrameRequest::FrameInfo &frame_info_proto) {
...
static PyObject *
convertSignalFrameRequestToDict(const SignalFrameRequest *signal_frame_proto) {
  PyObject *signal_frame_dict = PyDict_New();
  if (!signal_frame_dict)
    return NULL;
