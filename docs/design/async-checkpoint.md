...
many hours of the accelerator (GPU, NPU, TPU) since the training must stop until the checkpointing is completed.
On the other hand, too long checkpionting period will waste many training steps
when resuming the training from the latest checkpoint file. Shortening the checkpointing
