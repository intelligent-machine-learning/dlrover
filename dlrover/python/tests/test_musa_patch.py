...
class MusaPatchTest(unittest.TestCase):
    def setUp(self):
        ...
        modules_to_remove = [
            'dlrover.python.common.musa_patch',
            'torch_musa'
        ]
    ...
    def test_patch_after_import_torch_with_musa_available(self):
        ...
        with patch.dict('sys.modules', {'torch': mock_torch}):
            ...
            from dlrover.python.common.musa_patch import patch_after_import_torch
            ...
            self.assertEqual(mock_cuda.BFloat16Tensor, mock_musa.BFloat16Tensor)
            ...
            self.assertEqual(mock_cuda.current_stream, mock_musa.current_stream)
            self.assertEqual(mock_cuda.get_device_properties, mock_musa.get_device_properties)
            self.assertEqual(mock_cuda.is_bf16_supported, mock_musa.is_bf16_supported)
    ...
    def test_torch_musa_import_error(self, mock_print):
        ...
        mock_torch = Mock()
        mock_torch.musa = Mock()  # Add musa attribute to avoid AttributeError during patching
        mock_torch.cuda = Mock()
        mock_torch.Tensor = Mock()
        mock_torch.empty = Mock()
        
        with patch.dict('sys.modules', {'torch': mock_torch}), \
             patch('dlrover.python.common.musa_patch.torch_musa', side_effect=ImportError("No module named 'torch_musa'")):
            
            # This should handle the ImportError gracefully
            try:
                import dlrover.python.common.musa_patch
                ...
                self.fail("Module should handle torch_musa ImportError gracefully")
