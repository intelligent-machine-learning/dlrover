...
class NcclCheckRendezvousManagerTest(unittest.TestCase):
    def test_nccl_rdzv(self):
        ...
        rdzv_manager.report_nccl_check_result(0, True)
        rdzv_manager.report_nccl_check_result(1, True)
        rdzv_manager.report_nccl_check_result(2, True)
        rdzv_manager.report_nccl_check_result(3, False)
        self.assertFalse(rdzv_manager.nccl_check_success())
        world = rdzv_manager.get_comm_world(3, 2)
        self.assertDictEqual(world, {0: 8, 3: 8})
        rdzv_manager.report_nccl_check_result(3, True)
        self.assertTrue(rdzv_manager.nccl_check_success())
