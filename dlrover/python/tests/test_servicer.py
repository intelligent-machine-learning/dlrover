...
from dlrover.python.common.grpc_utils import GPUStats
...
class MasterServicerTest(unittest.TestCase):
    ...
    def test_query_running_nodes(self):
        ...
        message = grpc_utils.RunningNodesRequest()
        request.data = message.serialize()
        res = self.servicer.get(request, None)
        ret: grpc_utils.RunningNodes = grpc_utils.deserialize_message(res.data)
        self.assertEqual(len(ret.nodes), 3)

    def test_dataset_service(self):
        request = grpc_utils.DatasetShardParams()
        ...
        request = grpc_utils.TaskResult(dataset_name="test", task_id=0)
        request.task_id = 0
        request.dataset_name = "test"
        self.servicer._report_task_result(request)
        self.assertEqual(len(self.task_manager._datasets["test"].doing), 0)

        request = grpc_utils.ShardCheckpointRequest("test")
        request.dataset_name = "test"
    ...
    def test_metric_service(self):
        self.job_manager._init_nodes()
        self.job_manager._init_job_auto_scaler()
        request = grpc_utils.ResourceStats(gpu_stats=[])
        request.memory = 4096
        request.cpu = 2
        ...
        for gpu in gpu_stats:
            gpu_stats_message = grpc_utils.GPUStats()
    ...
    def test_get_cluster_version(self):
        message = grpc_utils.ClusterVersionRequest(NodeType.WORKER, 0, "local")
        request = elastic_training_pb2.Message()
        request.data = message.serialize()
        response = self.servicer.get(request, None)
        res_msg = grpc_utils.deserialize_message(response.data)
        self.assertEqual(res_msg.version, 0)

        message = grpc_utils.ClusterVersionRequest(NodeType.PS, 0, "local")
        request = elastic_training_pb2.Message()
        request.data = message.serialize()
        response = self.servicer.get(request, None)
        res_msg = grpc_utils.deserialize_message(response.data)
    ...
    def test_get_training_status(self):
        message = grpc_utils.TrainingStatusRequest()
        request = elastic_training_pb2.Message()
        request.data = message.serialize()
        response = self.servicer.get(request, None)
    ...
    def test_num_nodes_waiting(self):
        message = grpc_utils.WaitingNodeNumRequest(
    ...
    def test_report_task_result(self):
        request = elastic_training_pb2.Message()
        message = grpc_utils.TaskResult("test", 0, "error")
        dataset_params = grpc_utils.DatasetShardParams(
            batch_size=64,
            num_epochs=1,
            dataset_size=10000,
        ...
        message = grpc_utils.TaskResult("test", 0, "")
        request.data = message.serialize()
    ...
    def test_sync(self):
        request = elastic_training_pb2.Message()
        message = grpc_utils.SyncJoin("test")
        request.data = message.serialize()
        ...
        message = grpc_utils.SyncFinish("test")
        request.data = message.serialize()
        response = self.servicer.report(request, None)
        self.assertFalse(response.success)
        ...
        message = grpc_utils.SyncBarrier("test")
        request.data = message.serialize()
        response = self.servicer.report(request, None)
        self.assertFalse(response.success)
    ...
    def test_get_paral_config(self):
        message = grpc_utils.ParallelConfigRequest()
        request = elastic_training_pb2.Message()
        request.data = message.serialize()
        self.servicer.report(request, None)
        response = self.servicer.get(request, None)
        config = grpc_utils.deserialize_message(response.data)
        if config:
            self.assertIsInstance(config, grpc_utils.ParallelConfig)

    def test_get_straggler(self):
        message = grpc_utils.StragglerExistRequest()
        request = elastic_training_pb2.Message()
        request.data = message.serialize()
        self.servicer.report(request, None)
        response = self.servicer.get(request, None)
        config = grpc_utils.deserialize_message(response.data)
        self.assertIsInstance(config, grpc_utils.NetworkCheckResult)

    def test_check_hardware_reset(self):
        message = grpc_utils.CheckHardwareResetRequest()
        request = elastic_training_pb2.Message()
        request.data = message.serialize()
        response = self.servicer.get(request, None)
        config = grpc_utils.deserialize_message(response.data)
    ...
    def test_report_heartbeat(self):
        request = elastic_training_pb2.Message()
        ts = int(time.time())
        message = grpc_utils.HeartBeat(ts)
        request.data = message.serialize()
        request.node_type = NodeType.WORKER
        request.node_id = 0
    ...
    def test_sync_checkpoint(self):
        message = grpc_utils.NodeCheckpointState(step=100)
    ...
    def test_report_node_diagnosis_data(self):
        ...
        request = grpc_utils.DiagnosisReportData(
    ...
    def test_deal_with_reported_node_event(self):
        request = grpc_utils.NodeEvent(node=grpc_utils.NodeMeta())
        task_id = 1
