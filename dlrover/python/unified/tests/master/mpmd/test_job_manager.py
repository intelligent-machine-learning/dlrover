...
class JobManagerTest(BaseMasterTest):
    ...
    def test_basic(self):
        ...
        self.assertIsNone(job_manager.gen_failure_by_error())
        job_manager.gen_failure_by_error = MagicMock(
            return_value=FailureDesc(failure_obj="Trainer", reason="test")
        )
        self.assertEqual(job_manager.gen_failure_by_error().reason, "test")
