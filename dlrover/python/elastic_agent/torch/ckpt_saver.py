...
class AsyncCheckpointSaver(metaclass=ABCMeta):
    ...
    def start_async_saving_ckpt(cls):
        ...
        from the shared memory into the storage. Firstly, it waits the
        training process to notify the saver class to create a saver.
        """
    ...
    def save_shm_to_storage(self, timeout=60, master_client=None):
        ...
        step = steps[0]
        if master_client:
            # If some nodes failed, the living nodes do not need to save its
...
class CommonDirCheckpointSaver(AsyncCheckpointSaver):
    ...
    the step if the number of done files is equal to the number of shards.
    """
    ...
    def save_step_checkpoint(self, step: int):
        ...
        write_success = False
        # save to the stage path for each local rank
        futures: List[Future] = []
        for i in range(self.local_shard_num):
            default_config = CheckpointConfig()
            ckpt_config = self._shm_handlers[i].get_checkpoint_config(
    ...
    def commit_checkpoint(self, step: int, step_done_dir: str, timeout=600):
        """
        Node rank 0 will update the tracker file with the step
        after the number of done files is equal to the number of shards.
...
class TempDirCheckpointSaver(AsyncCheckpointSaver):
    ...
    def commit_checkpoint(  # type: ignore
        ...
        This method is called by agent rank 0, it will check if all agent ranks
