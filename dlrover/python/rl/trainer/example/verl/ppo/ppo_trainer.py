...
class PPOTrainer(BaseTrainer):
    ...
    def init(self):
        ...
        self.processor = hf_processor(local_path,
                                 use_fast=True)  # used for multimodal LLM, could be none
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_gen_batch_padded, pad_size = pad_dataproto_to_divisor(test_gen_batch, len(self.actors))
            test_output_gen_batch_padded = self.RG_ACTOR.generate_sequences(test_gen_batch_padded)
    ...
    def _save_checkpoint(self):
        ...
        self.RG_ACTOR.save_checkpoint(actor_local_path, actor_remote_path, self.global_steps, max_ckpt_to_keep=max_actor_ckpt_to_keep)

        if self.critics:
            critic_local_path = os.path.join(local_global_step_folder, "critic")
            critic_remote_path = None if self.config.trainer.default_hdfs_dir is None else os.path.join(self.config.trainer.default_hdfs_dir, f"global_step_{self.global_steps}", "critic")
            self.RG_CRITIC.save_checkpoint(critic_local_path, critic_remote_path, self.global_steps, max_ckpt_to_keep=max_critic_ckpt_to_keep)
    ...
    def _load_checkpoint(self):
        ...
        # load actor
        self.RG_ACTOR.load_checkpoint(actor_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load)
        # load critic
        if self.critics:
            self.RG_CRITIC.load_checkpoint(critic_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        if not self.async_rollout_mode:
                            gen_batch_output = self.RG_ACTOR.generate_sequences(gen_batch)
                        else:
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        with _timer("gen_max", timing_raw):
                            ...
                            gen_baseline_output = self.RG_ACTOR.generate_sequences(gen_baseline_batch)
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if self.rewards:
                            reward_tensor = self.RG_REWARD.compute_rm_score(batch)
                    ...
                    if self.references:
                        ...
                        with _timer("ref", timing_raw):
                            ref_log_prob = self.RG_REFERENCE.compute_ref_log_prob(batch)
                ...
                # collect metrics
                metrics.update(compute_data_metrics(batch=batch, use_critic=(len(self.critics) > 0)))
