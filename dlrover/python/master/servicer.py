...
class MasterServicer(elastic_training_pb2_grpc.MasterServicer):
    ...
    def get(self, request, _):
        ...
        req_message = grpc_utils.deserialize_message(request.data)

        response = elastic_training_pb2.Message()
        if not req_message:
            return response
        message = None
        if isinstance(req_message, grpc_utils.TaskRequest):
            message = self._get_task(node_type, node_id, req_message)
        elif isinstance(req_message, grpc_utils.ShardCheckpointRequest):
            message = self._get_shard_checkpoint(req_message)
        elif isinstance(req_message, grpc_utils.ClusterVersionRequest):
            message = self._get_cluster_version(req_message)
        elif isinstance(req_message, grpc_utils.RunningNodesRequest):
            message = self._get_running_nodes()
        elif isinstance(req_message, grpc_utils.JoinRendezvousRequest):
            message = self._join_rendezvous(req_message)
        elif isinstance(req_message, grpc_utils.WaitingNodeNumRequest):
            message = self._num_nodes_waiting(req_message.rdzv_name)
        elif isinstance(req_message, grpc_utils.NetworkReadyRequest):
            message = self._check_fault_node()
        elif isinstance(req_message, grpc_utils.StragglerExistRequest):
            message = self._check_straggler()
        elif isinstance(req_message, grpc_utils.CommWorldRequest):
            message = self._get_comm_world(req_message)
        elif isinstance(req_message, grpc_utils.KeyValuePair):
            message = self._kv_store_get(req_message)
        elif isinstance(req_message, grpc_utils.PsNodesRequest):
            message = self._query_ps_nodes()
        elif isinstance(req_message, grpc_utils.TrainingStatusRequest):
            message = self._get_training_status()
        elif isinstance(req_message, grpc_utils.ParallelConfigRequest):
            message = self._get_paral_config()
        elif isinstance(req_message, grpc_utils.CheckHardwareResetRequest):
            message = self._need_to_restart_training(node_type, node_id)
        elif isinstance(req_message, grpc_utils.SyncTrainingPort):
            message = self._sync_training_ports(node_id, req_message)
        elif isinstance(req_message, grpc_utils.ElasticRunConfigRequest):
            configs = self._job_manager.get_elastic_run_configs()
            message = grpc_utils.ElasticRunConfig(configs=configs)
        elif isinstance(req_message, grpc_utils.HeartBeat):
    ...
    def _get_task(self, node_type, node_id, request: grpc_utils.TaskRequest):
        if not self._start_training_time:
            self._start_training_time = int(time.time())
        shard = grpc_utils.Shard()
        res = grpc_utils.Task(shard=shard)
    ...
    def _get_cluster_version(self, request: grpc_utils.ClusterVersionRequest):
        message = grpc_utils.ClusterVersion()
    ...
    def _get_running_nodes(self):
        res = grpc_utils.RunningNodes(nodes=[])
        nodes: List[Node] = self._job_manager.get_running_nodes()
        for node in nodes:
            meta = grpc_utils.NodeMeta()
            meta.type = node.type
    ...
    def _check_fault_node(self):
        ...
        res = grpc_utils.NetworkCheckResult(nodes=nodes, reason=reason)
        return res
    ...
    def _join_rendezvous(self, request: grpc_utils.JoinRendezvousRequest):
        ...
        res = grpc_utils.RendezvousState(round=round)
        return res

    def _num_nodes_waiting(self, rdzv_name):
        waiting_num = self._rdzv_managers[rdzv_name].num_nodes_waiting()
        res = grpc_utils.RendezvousState(waiting_num=waiting_num)
        return res

    def _get_comm_world(self, request: grpc_utils.CommWorldRequest):
        rdzv_manager = self._rdzv_managers[request.rdzv_name]
        rdzv_round, group, nodes = rdzv_manager.get_comm_world(request.node_id)
        res = grpc_utils.RendezvousState(world={})
        res.group = group
    ...
    def _kv_store_get(self, request: grpc_utils.KeyValuePair):
        value = self._kv_store.get(request.key)
        res = grpc_utils.KeyValuePair(request.key, value)
        return res

    def _get_paral_config(self):
        res = self._job_manager.get_opt_strategy()
        if not res:
            res = grpc_utils.ParallelConfig()
        return res
    ...
    def report(self, request, _):
        node_type = request.node_type
        node_id = request.node_id
        message = grpc_utils.deserialize_message(request.data)
        ...
        success = False
        if isinstance(message, grpc_utils.DatasetShardParams):
            success = self._collect_dataset_shard_params(message)
        elif isinstance(message, grpc_utils.ResourceStats):
            success = self._update_node_resource_usage(
                node_type, node_id, message
            )
        elif isinstance(message, grpc_utils.ModelInfo):
            success = self._collect_model_info(message)
        elif isinstance(message, grpc_utils.GlobalStep):
            success = self._collect_global_step(message)
        elif isinstance(message, grpc_utils.ShardCheckpoint):
            success = self._restore_shard_checkpoint(message)
        elif isinstance(message, grpc_utils.TaskResult):
            success = self._report_task_result(message)
        elif isinstance(message, grpc_utils.ClusterVersion):
            success = self._update_cluster_version(message)
        elif isinstance(message, grpc_utils.NodeAddress):
            success = self._update_node_address(message)
        elif isinstance(message, grpc_utils.NodeEvent):
            success = self._deal_with_reported_node_event(message)
        elif isinstance(message, grpc_utils.SyncJoin):
            success = self._join_sync(node_type, node_id, message)
        elif isinstance(message, grpc_utils.SyncFinish):
            success = self._sync_finished(message)
        elif isinstance(message, grpc_utils.SyncBarrier):
            success = self._barrier(message)
        elif isinstance(message, grpc_utils.NodeFailure):
            success = self._report_failure(node_type, node_id, message)
        elif isinstance(message, grpc_utils.RendezvousParams):
            success = self._report_rdzv_params(message)
        elif isinstance(message, grpc_utils.PsReady):
            success = self._ready_for_ps_relaunch()
        elif isinstance(message, grpc_utils.KeyValuePair):
            success = self._kv_store_set(message)
        elif isinstance(message, grpc_utils.ParallelConfig):
            success = self._report_paral_config(node_type, node_id, message)
        elif isinstance(message, grpc_utils.NodeCheckpointState):
            success = self._sync_checkpoint(node_type, node_id, message)
        elif isinstance(message, grpc_utils.DiagnosisReportData):
            success = self._report_node_diagnosis_data(message)
        elif isinstance(message, grpc_utils.Event):
    ...
    def _update_node_resource_usage(
        self, node_type, node_id, metrics: grpc_utils.ResourceStats
    ):
        logger.debug(
    ...
    def _collect_model_info(self, metrics: grpc_utils.ModelInfo):
        if self._job_metric_collector:
            self._job_metric_collector.collect_model_metric(metrics)
        return True

    def _collect_global_step(self, metrics: grpc_utils.GlobalStep):
    ...
    def _restore_shard_checkpoint(self, message: grpc_utils.ShardCheckpoint):
    ...
    def _report_task_result(self, request: grpc_utils.TaskResult):
        success = True
    ...
    def _update_cluster_version(self, message: grpc_utils.ClusterVersion):
        if not self._elastic_ps_service:
            return False
    ...
    def _update_node_address(self, message: grpc_utils.NodeAddress):
    ...
    def _deal_with_reported_node_event(self, message: grpc_utils.NodeEvent):
        node = Node(
    ...
    def _join_sync(self, node_type, node_id, message: grpc_utils.SyncJoin):
        success = False
    ...
    def _sync_finished(self, message: grpc_utils.SyncFinish):
        success = False
        if self._sync_service:
            success = self._sync_service.sync_finished(message.sync_name)
        return success

    def _barrier(self, message: grpc_utils.SyncBarrier):
    ...
    def _report_rdzv_params(self, message: grpc_utils.RendezvousParams):
    ...
    def _kv_store_set(self, message: grpc_utils.KeyValuePair):
        self._kv_store.set(message.key, message.value)
        return True

    def _report_paral_config(
        self, node_type, node_id, message: grpc_utils.ParallelConfig
    ):
        if self._job_manager:
            logger.debug(
    ...
    def _sync_checkpoint(
        self, node_type, node_id, message: grpc_utils.NodeCheckpointState
    ):
    ...
    def _sync_training_ports(
        self, node_id, message: grpc_utils.SyncTrainingPort
    ) -> grpc_utils.SyncTrainingPort:
        ...
        return grpc_utils.SyncTrainingPort(
            port=sync_ports.training_port, newport=sync_ports.next_check_port
        )

    def _report_event(self, message: grpc_utils.Event):
    ...
    def _report_heartbeat(
        self, node_type, node_id, message: grpc_utils.HeartBeat
    ) -> grpc_utils.HeartbeatResponse:
        action = self._job_manager.collect_node_heart_beat(
            node_type, node_id, message.timestamp
        )
        grpc_action = grpc_utils.DiagnosisAction(
            action.__class__.__name__,
            action.to_json(),
        )
        return grpc_utils.HeartbeatResponse(action=grpc_action)
