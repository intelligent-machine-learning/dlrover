...
func (m *Manager) Run(ctx context.Context, errReporter common.ErrorReporter) error {
	...
	if !success {
		return errors.New("failed to get config when initialization")
	}
	return nil
}
...
// Get the latest config from config map directly.
...
// Retrieve server config from ConfigMap by config key.
