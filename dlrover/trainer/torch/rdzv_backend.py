...
from dlrover.python.elastic_agent.master_client import GlobalMasterClient, MasterClient
...
class DlroverRendezvousBackend(RendezvousBackend):
    ...
    def get_state(self) -> Optional[Tuple[bytes, Token]]:
        """See base class."""
        try:
            result = self._client.get_rdzv_state()
    ...
    def set_state(
        ...
        try:
            result = self._client.set_rdzv_state(base64_state)
        except grpc.RpcError as exc:
            raise RendezvousConnectionError(
                "The connection to etcd has failed. See inner exception for details."
            ) from exc
