...
from py_xpu_timer.version import __build_platform_version__
...
def support_cupti():
    return "event" not in __build_platform_version__
...
class TracingData:
    ...
    def parse_one_trace_data(args):
        ...
        with path.open("rb") as f:
            ...
            for each_trace in timeline_traces.traces:
                debug_data = each_trace.debug_data
                tracing = TracingData(
                    path,
                    each_trace,
                    debug_data,
    ...
    def add_annotation(self, debug_annotations, matmul_debug_info, packet):
        ...
        # launch data
        launch_data = self.debug_data.nvidia_launch_data if self.debug_data.HasField("nvidia_launch_data") else None
        if launch_data:
            if not support_cupti():
                if self.debug_data.HasField("nccl_debug"):
                    grids = debug_annotations.add()
                    grids.name_iid = 8  # grids
                    grids.string_value = f"[{','.join(map(str,launch_data.grids))}]"

                    blocks = debug_annotations.add()
                    blocks.name_iid = 7  # block
                    blocks.string_value = f"[{','.join(map(str,launch_data.blocks))}]"
            else:
                stream_id = debug_annotations.add()
                stream_id.name_iid = 29  # stream_id
                stream_id.uint_value = launch_data.stream_id

                if not self.debug_data.HasField("memory_debug"):
                    grids = debug_annotations.add()
                    grids.name_iid = 8  # grids
                    grids.string_value = f"[{','.join(map(str,launch_data.grids))}]"
        ...
        # kernel data
        if self.debug_data.HasField("mm_debug"):
            kernel_data = self.debug_data.mm_debug

            flop = 2
            # bmnk
            for field, value in zip("bmnk", kernel_data.shapes):
            ...
            # lds
            for field, value in zip(range(15, 18), kernel_data.lds):
                annotation = debug_annotations.add()
                annotation.name_iid = field
                annotation.uint_value = value

            # strides
            for field, value in zip(range(18, 21), kernel_data.strides):
                annotation = debug_annotations.add()
                annotation.name_iid = field
                annotation.uint_value = value
            cublas_api = debug_annotations.add()
            cublas_api.name_iid = 13
            cublas_api.string_value_iid = TracingData.debug_annotation_string_values[kernel_data.api]

            cublas_algo = debug_annotations.add()
            cublas_algo.name_iid = 21  # cublas algo
            cublas_algo.int_value = kernel_data.algo

            trans = debug_annotations.add()
            trans.name_iid = 22  # trans
            trans.string_value = kernel_data.trans

            dtype = debug_annotations.add()
            dtype.name_iid = 9  # dtype
            dtype.string_value = kernel_data.dtype

            tflops = debug_annotations.add()
            tflops.name_iid = 10  # tflops
            tflops.double_value = round(flop / self.dur / 1e6, 2)
            matmul_debug_info[kernel_data.SerializeToString()].append(tflops.double_value)

        elif self.debug_data.HasField("fa_debug"):
            kernel_data = self.debug_data.fa_debug

            for field, value in zip("bssh", kernel_data.shapes):
                annotation = debug_annotations.add()
                annotation.name = field
                annotation.uint_value = value

        elif self.debug_data.HasField("grouped_mm_debug"):
            kernel_data = self.debug_data.grouped_mm_debug

            tflops = debug_annotations.add()
            tflops.name_iid = 10
            tflops.double_value = round(kernel_data.tflops / self.dur / 1e6, 2)

        elif self.debug_data.HasField("nccl_debug"):
            kernel_data = self.debug_data.nccl_debug

            comm_hash = debug_annotations.add()
            comm_hash.name_iid = 3  # hash
            comm_hash.uint_value = kernel_data.comm_hash

            input_size = debug_annotations.add()
            input_size.name_iid = 6  # bytes
            input_size.uint_value = kernel_data.input_size_in_bytes
            ...
            nranks.uint_value = kernel_data.ranks

            nodes = debug_annotations.add()
            nodes.name_iid = 5  # nodes
            nodes.uint_value = kernel_data.nodes

            seq = debug_annotations.add()
            seq.name_iid = 11  # seq num
            seq.uint_value = kernel_data.seq

            bandwidth = debug_annotations.add()
            bandwidth.name_iid = 24  # GiB/s
            bandwidth.double_value = round(kernel_data.problem_size / (1 << 30) / self.dur * 1e6, 2)

            if kernel_data.send_recv_type < 2:
                send_recv_type = debug_annotations.add()
                send_recv_type.name_iid = 28
                send_recv_type.string_value = "NcclSend" if kernel_data.send_recv_type == 0 else "NcclRecv"

        elif self.debug_data.HasField("memory_debug"):
            kernel_data = self.debug_data.memory_debug

            direction = debug_annotations.add()
            direction.name_iid = 23  # direction
            direction.string_value = kernel_data.direction
            bandwidth = debug_annotations.add()
            bandwidth.name_iid = 24  # GiB/s
            bandwidth.double_value = round(kernel_data.size / (1 << 30) / self.dur * 1e6, 2)
            copy_bytes = debug_annotations.add()
            copy_bytes.name_iid = 25  # bytes
            copy_bytes.uint_value = kernel_data.size
        else:
            raise ValueError(f"Debug data shoule be FA/Matmul/Nccl/Memory, but got {self.debug_data}")
    ...
    def gen_flow_id(self):
        if self.kernel_type == "nccl":
            # self.debug_data.nccl_debug.seq is less than 0xFFFFFFFF
            high_bits = self.debug_data.nccl_debug.seq << 32
            low_bits = (self.debug_data.nccl_debug.comm_hash >> 32) & 0xFFFFFFFF
