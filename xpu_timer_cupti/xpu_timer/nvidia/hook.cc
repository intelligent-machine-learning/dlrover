...
ncclResult_t ncclAllReduce(const void* sendbuff, void* recvbuff, size_t count,
  ...
  auto fn = xpu_timer::GpuTimerManager<
                xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                .intercept_manager.handleNcclCupti(
                    count, datatype, comm, stream,
                    xpu_timer::constant::CollType::AllReduce);
  auto event = xpu_timer::GpuTimerManager<
                   xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                   .getEvent();
  event->reset(stream, fn, xpu_timer::constant::Metrics::CollMetrics::TYPE);
  auto status =
      orig_ncclAllReduce(sendbuff, recvbuff, count, datatype, op, comm, stream);
...
ncclResult_t ncclReduce(const void* sendbuff, void* recvbuff, size_t count,
  ...
  auto fn = xpu_timer::GpuTimerManager<
                ...
                .intercept_manager.handleNcclCupti(
                    count, datatype, comm, stream,
                    xpu_timer::constant::CollType::Reduce);
  auto event = xpu_timer::GpuTimerManager<
                   xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                   .getEvent();
  event->reset(stream, fn, xpu_timer::constant::Metrics::CollMetrics::TYPE);
  auto status = orig_ncclReduce(sendbuff, recvbuff, count, datatype, op, root,
                                comm, stream);
...
ncclResult_t ncclAllGather(const void* sendbuff, void* recvbuff,
  ...
  auto fn = xpu_timer::GpuTimerManager<
                ...
                .intercept_manager.handleNcclCupti(
                    ...
                    xpu_timer::constant::CollType::AllGather);
  auto event = xpu_timer::GpuTimerManager<
                   xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                   .getEvent();
  event->reset(stream, fn, xpu_timer::constant::Metrics::CollMetrics::TYPE);
  auto status =
      orig_ncclAllGather(sendbuff, recvbuff, sendcount, datatype, comm, stream);
...
ncclResult_t ncclReduceScatter(const void* sendbuff, void* recvbuff,
  ...
  auto fn = xpu_timer::GpuTimerManager<
                ...
                .intercept_manager.handleNcclCupti(
                    ...
                    xpu_timer::constant::CollType::ReduceScatter);
  auto event = xpu_timer::GpuTimerManager<
                   xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                   .getEvent();
  event->reset(stream, fn, xpu_timer::constant::Metrics::CollMetrics::TYPE);
  auto status = orig_ncclReduceScatter(sendbuff, recvbuff, recvcount, datatype,
                                       op, comm, stream);
...
ncclResult_t ncclSend(const void* sendbuff, size_t count,
  ...
  auto fn = xpu_timer::GpuTimerManager<
                ...
                .intercept_manager.handleNcclCupti(
                    count, datatype, comm, stream,
                    xpu_timer::constant::CollType::Send);
  auto event = xpu_timer::GpuTimerManager<
                   xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                   .getEvent();
  event->reset(stream, fn, xpu_timer::constant::Metrics::CollMetrics::TYPE);
  auto status = orig_ncclSend(sendbuff, count, datatype, peer, comm, stream);
...
ncclResult_t ncclRecv(void* recvbuff, size_t count, ncclDataType_t datatype,
  ...
  auto fn = xpu_timer::GpuTimerManager<
                ...
                .intercept_manager.handleNcclCupti(
                    count, datatype, comm, stream,
                    xpu_timer::constant::CollType::Recv);
  auto event = xpu_timer::GpuTimerManager<
                   xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                   .getEvent();
  event->reset(stream, fn, xpu_timer::constant::Metrics::CollMetrics::TYPE);
  auto status = orig_ncclRecv(recvbuff, count, datatype, peer, comm, stream);
...
ncclResult_t ncclBroadcast(const void* sendbuff, void* recvbuff, size_t count,
  ...
  auto fn = xpu_timer::GpuTimerManager<
                ...
                .intercept_manager.handleNcclCupti(
                    count, datatype, comm, stream,
                    xpu_timer::constant::CollType::Broadcast);
  auto event = xpu_timer::GpuTimerManager<
                   xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                   .getEvent();
  event->reset(stream, fn, xpu_timer::constant::Metrics::CollMetrics::TYPE);
  auto status = orig_ncclBroadcast(sendbuff, recvbuff, count, datatype, root,
                                   comm, stream);
...
ncclResult_t ncclGroupStart() {
  SETUP_DLSYM_WITH_NCCL(ncclGroupStart);
  if (!::xpu_timer::util::config::GlobalConfig::enable) {
    return orig_ncclGroupStart();
  }
  auto status = orig_ncclGroupStart();
  ++xpu_timer::nvidia::NvidiaGpuTimer::nccl_group_start_count;
  return status;
}

EXPOSE_API
ncclResult_t ncclGroupEnd() {
  SETUP_DLSYM_WITH_NCCL(ncclGroupEnd);
  if (!::xpu_timer::util::config::GlobalConfig::enable) {
    return orig_ncclGroupEnd();
  }
  xpu_timer::nvidia::NvidiaGpuTimer::nccl_group_start_count -= 1;

  if ((xpu_timer::nvidia::NvidiaGpuTimer::nccl_group_start_count == 0) &&
      (xpu_timer::nvidia::NvidiaGpuTimer::nccl_group_coll_queue.size() != 0)) {
    auto fn = xpu_timer::GpuTimerManager<
                  xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                  .intercept_manager.handlNcclGroupEnd();
    auto event = xpu_timer::GpuTimerManager<
                     xpu_timer::nvidia::NvidiaGpuTimer>::getInstance()
                     .getEvent();

    event->reset(
        xpu_timer::nvidia::NvidiaGpuTimer::nccl_group_coll_queue[0]->stream_,
        fn, xpu_timer::constant::Metrics::CollMetrics::TYPE);

    for (xpu_timer::nvidia::NcclGroupCollInfo* coll_info :
         xpu_timer::nvidia::NvidiaGpuTimer::nccl_group_coll_queue) {
      delete coll_info;
    }
    xpu_timer::nvidia::NvidiaGpuTimer::nccl_group_coll_queue.clear();

    auto status = orig_ncclGroupEnd();
