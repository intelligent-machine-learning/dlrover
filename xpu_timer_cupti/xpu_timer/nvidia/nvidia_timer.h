...
struct alignas(64) NvidiaLaunchInfo {
  uint64_t start_timestamp_;  // us
  uint64_t duration_;         // us
  uint32_t stream_id_;
  dim3 grid_dim_;
  dim3 block_dim_;
  char padding[64 - 2 * sizeof(uint64_t) - 2 * sizeof(dim3) - sizeof(uint32_t)];
  NvidiaLaunchInfo() = default;

  NvidiaLaunchInfo(uint64_t start_timestamp, uint64_t duration,
                   uint32_t stream_id, dim3 grid_dim = dim3(0, 0, 0),
  ...
  NvidiaLaunchInfo(const NvidiaLaunchInfo&) = default;
  NvidiaLaunchInfo& operator=(const NvidiaLaunchInfo&) = default;
};

struct NcclGroupCollInfo {
  uint64_t comm_hash;
  ...
  uint32_t ranks;
  std::string dtype;
  NcclGroupCollInfo(uint64_t comm_hash, cudaStream_t stream, std::string name,
                    uint32_t comm_size, uint64_t problem_size, uint32_t nodes,
                    uint32_t ranks, std::string dtype)
      : comm_hash(comm_hash),
        stream_(stream),
        name_(name),
        comm_size(comm_size),
...
class NvidiaGpuTimer : public XpuTimer {
 ...
 public:
  ...
  // ID of cupti external correlation
  static std::vector<xpu_timer::nvidia::NcclGroupCollInfo*>
      nccl_group_coll_queue;
  thread_local static uint32_t nccl_group_start_count;

  explicit NvidiaGpuTimer() {
#ifdef NVIDIA_WITH_CUPTI
    cudaEventCreateWithFlags(&start_event_, cudaEventDisableTiming);
    cudaEventCreateWithFlags(&stop_event_, cudaEventDisableTiming);
#else
  ...
  static void doDestroy();  // do some destroy works, maybe release hooks from
                            // CuptiActivity.
  ...
  // nvidia launch info
  NvidiaLaunchInfo nvidia_launch_info_;
 ...
 private:
  ...
  // Counter for which EndRecord is ready, but Cupit has not yet.
  uint64_t event_ready_counter_;
  // ID of cupti external correlation
  uint32_t cupti_external_id_;
  // whether the current coll op is in the nccl group
  bool is_in_nccl_group_;
  // The kernel may consist of 2 parts, such as the splitk reduce algorithm.
  // Getting the first cupti_id may not be ready.
  bool may_ready;
};
...
class InterceptManager {
 public:
  enum SendRecvType {
    Send = 0,          // send
    Recv = 1,          // recv
    NoSendOrRecv = 2,  // no send or recv
  };
 ...
 public:
  ...
  std::function<NvidiaGpuTimer::FnReturn(NvidiaGpuTimer*)> handleNcclCupti(
      size_t count, ncclDataType_t datatype, ncclComm_t orig_comm,
      cudaStream_t stream, constant::CollType coll_type);
  std::function<NvidiaGpuTimer::FnReturn(NvidiaGpuTimer*)> handlNcclGroupEnd();
