...
InterceptManager::handleNccl(
...
#if defined(XPU_NPU)
  ncclWorkElem* work = (ncclWorkElem*)(dev_comm);
  dev_comm = (void*)(work->comm);
#endif
  ...
  auto fn = [sym,
    ...
    hook::NcclDebugData* kernel_data = debug_data->mutable_nccl_debug();

#if defined(CUDA_LAUNCH_EXC)
    launch_data->add_grids(grid_dim.x);
    launch_data->add_grids(grid_dim.y);
    launch_data->add_grids(grid_dim.z);
    launch_data->add_blocks(block_dim.x);
    launch_data->add_blocks(block_dim.y);
    launch_data->add_blocks(block_dim.z);
#endif
    ...
    kernel_data->set_comm_hash(nccl_info.comm->commHash);
    kernel_data->set_send_recv_type(static_cast<int>(nccl_info.send_recv_type));
    kernel_data->set_input_size_in_bytes(comm_size);
    kernel_data->set_dtype(dtype);
    kernel_data->set_ranks(nranks);
    kernel_data->set_nodes(comm->nNodes);
    kernel_data->set_seq(++(timer->nccl_seq_num[nccl_info.comm->commHash]));
    kernel_data->set_problem_size(problem_size);
...
InterceptManager::handleFa(void** args, const InterceptSymbol* sym) {
  ...
  auto fn = [sym,
    ...
    hook::NvidiaLaunchData* launch_data =
        debug_data->mutable_nvidia_launch_data();
    hook::FaDebugData* kernel_data = debug_data->mutable_fa_debug();
    ...
    for (int i = 0; i < 4; i++) {
      oss << fa_params[i] << "_";
      kernel_data->add_shapes(fa_params[i]);
    }
    uint64_t flops;
...
InterceptManager::deviceMemory(const std::string& name, const size_t size,
  ...
  return [name, size, kind](NvidiaGpuTimer* timer) {
    timer->trace->Clear();
    timer->trace->set_kernel_type(
    ...
    hook::NvidiaLaunchData* launch_data =
        debug_data->mutable_nvidia_launch_data();
    hook::MemoryDebugData* kernel_data = debug_data->mutable_memory_debug();

    launch_data->set_stream_id(timer->nvidia_launch_info_.stream_id_);

    kernel_data->set_size(size);
    if (!kind.empty()) {
      kernel_data->set_direction(kind);
...
std::function<NvidiaGpuTimer::FnReturn(NvidiaGpuTimer*)>
InterceptManager::handleNcclCupti(size_t count, ncclDataType_t datatype,
                                  ncclComm_t orig_comm, cudaStream_t stream,
                                  constant::CollType coll_type) {
  auto calculate_problem_size_fn = [](const constant::CollType& coll_type,
                                      const uint64_t& comm_size,
                                      const int& num_ranks) -> auto {
    double factor = 1.0;
    switch (coll_type) {
      case constant::CollType::AllReduce:
        factor = 2.0 * (num_ranks - 1) / num_ranks;
        break;
      case constant::CollType::AllGather:
      case constant::CollType::ReduceScatter:
        // input of reduce_scatter/allgather is sharded, so we do not device
        // world_size
        factor = static_cast<double>(num_ranks - 1);
        break;
      default:
        break;
    }
  ...
  if (NvidiaGpuTimer::nccl_group_start_count) {
    auto comm = std::make_shared<NcclCommWrapper>(orig_comm);
    int num_ranks = comm->nRanks;
    std::string dtype = CudaDataTypeUtils::getNcclDataType(datatype);
    uint64_t comm_size = count * CudaDataTypeUtils::getDtypeSizeInBytes(dtype);
    uint64_t problem_size =
        calculate_problem_size_fn(coll_type, comm_size, num_ranks);

    xpu_timer::nvidia::NcclGroupCollInfo* nccl_group_coll_info =
        new xpu_timer::nvidia::NcclGroupCollInfo(
            comm->commHash, stream,
            xpu_timer::util::collTypeToString(coll_type), comm_size,
            problem_size, comm->nNodes, comm->nRanks, dtype);

    xpu_timer::nvidia::NvidiaGpuTimer::nccl_group_coll_queue.push_back(
        nccl_group_coll_info);
  }
  auto fn = [orig_comm, datatype, count, coll_type, calculate_problem_size_fn](
                NvidiaGpuTimer* timer) -> NvidiaGpuTimer::FnReturn {
    timer->trace->Clear();
    timer->trace->set_kernel_type(constant::Metrics::CollMetrics::KERNEL_TYPE);
    ...
    std::ostringstream oss;
    oss << "xpu_timer_" << util::collTypeToString(coll_type) << "_size_"
        << comm_size;
    ...
    uint64_t problem_size =
        calculate_problem_size_fn(coll_type, comm_size, nranks);

    kernel_data->set_comm_hash(comm->commHash);
    kernel_data->set_send_recv_type(static_cast<int>(coll_type));
    kernel_data->set_input_size_in_bytes(comm_size);
    kernel_data->set_dtype(dtype);
    kernel_data->set_ranks(nranks);
    kernel_data->set_nodes(comm->nNodes);
    kernel_data->set_seq(++(timer->nccl_seq_num[comm->commHash]));
    kernel_data->set_problem_size(problem_size);
    ...
    return std::make_tuple(
        ...
        xpu_timer::Labels{
            {"dtype", dtype},
            {"operation", util::collTypeToString(coll_type)},
            {"transport", comm->nNodes > 1 ? "InterNode" : "IntraNode"}});
  };
  return fn;
}
...
std::function<NvidiaGpuTimer::FnReturn(NvidiaGpuTimer*)>
InterceptManager::handlNcclGroupEnd() {
  std::stringstream dtype;
  uint64_t comm_size = 0;
  uint64_t problem_size = 0;
  uint64_t comm_hash = NvidiaGpuTimer::nccl_group_coll_queue[0]->comm_hash;
  uint64_t nodes = NvidiaGpuTimer::nccl_group_coll_queue[0]->nodes;
  uint64_t ranks = NvidiaGpuTimer::nccl_group_coll_queue[0]->ranks;

  for (auto coll_info : NvidiaGpuTimer::nccl_group_coll_queue) {
    dtype << coll_info->name_ << "_" << coll_info->dtype << "_"
          << coll_info->comm_size;
    dtype << ",";

    comm_size += coll_info->comm_size;
    problem_size += coll_info->problem_size;
  }
  std::string nccl_group_name = dtype.str();

  auto fn = [nccl_group_name, comm_hash, comm_size, problem_size, nodes,
             ranks](NvidiaGpuTimer* timer) -> NvidiaGpuTimer::FnReturn {
    timer->trace->Clear();
    timer->trace->set_kernel_type(constant::Metrics::CollMetrics::KERNEL_TYPE);
    ...
    std::ostringstream oss;
    oss << "xpu_timer_nccl_group_operation";

    kernel_data->set_comm_hash(comm_hash);
    kernel_data->set_send_recv_type(
        static_cast<int>(SendRecvType::NoSendOrRecv));
    kernel_data->set_input_size_in_bytes(comm_size);
    kernel_data->set_dtype(nccl_group_name);
    kernel_data->set_ranks(ranks);
    kernel_data->set_nodes(nodes);
    kernel_data->set_seq(++(timer->nccl_seq_num[comm_hash]));
    kernel_data->set_problem_size(problem_size);

    uint64_t problem_size_bits = problem_size * 8;

    return std::make_tuple(oss.str(), problem_size_bits,
                           xpu_timer::Labels{{"operation", "nccl_group"}});
  };
  return fn;
}
