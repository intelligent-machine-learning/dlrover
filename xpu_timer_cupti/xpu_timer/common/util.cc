...
void setUpDlopenLibrary() {
...
#if defined(XPU_HPU)
  bool ascend_cl = false;
  bool ascend_hccl = false;
#endif
  ...
  while (std::getline(maps_file, line)) {
    ...
    if (pos != std::string::npos) {
      ...
      if (!ascend_cl && path.find("libascendcl.so") != std::string::npos) {
        std::string ascend_cl_name = "libascendcl.so";
        std::string opapi_name = "libopapi.so";
        path.replace(path.end() - ascend_cl_name.length(), path.end(),
                     opapi_name);
        GlobalConfig::dlopen_path["HPU_OPAPI"] = path;
      } else if (!ascend_hccl && path.find("libhccl.so") != std::string::npos) {
        GlobalConfig::dlopen_path["HPU_HCCL"] = path;
        ascend_hccl = true;
      }
      if (ascend_cl && ascend_hccl) {
        break;
      }
#endif
    }
...
void setUpGlobalConfig() {
  ...
  std::string dev_path = "/dev/alixpu_npu";
#elif defined(XPU_HPU)
...
std::string collTypeToString(constant::CollType coll_type) {
  switch (coll_type) {
    case constant::CollType::AllReduce:
      return "AllReduce";
    case constant::CollType::Reduce:
      return "Reduce";
    case constant::CollType::AllGather:
      return "AllGather";
    case constant::CollType::ReduceScatter:
      return "ReduceScatter";
    case constant::CollType::Send:
      return "Send";
    case constant::CollType::Recv:
      return "Recv";
    case constant::CollType::Broadcast:
      return "Broadcast";
    default:
      return "undefined";
  }
