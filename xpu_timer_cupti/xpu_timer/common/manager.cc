...
using XPU_TIMER = xpu_timer::hpu::HpuTimer;
#endif
...
bool KernelTraceManager::pushTrace(XPU_TIMER* work_item) {
  ...
  if (uint64_t(work_item->getLaunchTimeStamp()) <
      switch_->getObj()->timestamp * constant::US_TO_NS) {
    return true;
  }
...
void GpuTimerManager<XPU_TIMER>::doHang() {
  ...
  metrics_manager_->pushCommonMetricsToRemote("hang", 1,
                                              constant::Metrics::GAUGE);
  metrics_manager_->pushCommonMetricsToRemote("start_dump", 1,
                                              constant::Metrics::GAUGE);
  ...
  metrics_manager_->pushCommonMetricsToRemote("end_dump", duration.count(),
                                              constant::Metrics::GAUGE);
  bool exit =
...
void GpuTimerManager<XPU_TIMER>::doWork() {
  ...
  auto is_skipped_fn = [&is_skipped](XPU_TIMER* value) -> bool {
  ...
  while (should_run_.load()) {
    ...
    XPU_TIMER* work_item = working_queue_.pop(is_hang_fn, is_ready_fn,
                                              is_skipped_fn, &work_queue_size);
    if (!work_item) {
      need_sleep = true;
      continue;
    }
    if (is_hang) {
      doHang();
#ifdef XPU_HPU
      //  Using aclrtQueryEventStatus may cause a hang; if a hang occurs, we
      //  will skip this event.
      if (work_item->py_stack) {
        py_stack_util_->insertPyStack(work_item->getName(),
                                      *work_item->py_stack);
    ...
    work_item->reBuild();
#ifdef XPU_HPU
    // If aclrtQueryEventStatus indicates the host device has completed, but
    // aclrtSynchronizeEventWithTimeout returns as not completed, and
    // aclrtEventElapsedTime cannot retrieve the correct time, we set the
    // elapsed time to zero when an API error occurs.
    if (work_item->getDuration() == 0) {
      if (work_item->py_stack) {
        py_stack_util_->insertPyStack(work_item->getName(),
                                      *work_item->py_stack);
        delete work_item->py_stack;
    ...
    if (loop_count_ % 50000 == 0) {  // about 10 seconds
      ...
      // TODO(zhangji.zhang) batch rpc
      if (!is_hang) {
        metrics_manager_->pushCommonMetricsToRemote("hang", 0,
                                                    constant::Metrics::GAUGE);
        metrics_manager_->pushCommonMetricsToRemote("start_dump", 0,
                                                    constant::Metrics::GAUGE);
        metrics_manager_->pushCommonMetricsToRemote("end_dump", 0,
                                                    constant::Metrics::GAUGE);
      }
      metrics_manager_->pushCommonMetricsToRemote(
          "pool_queue_size", pool_queue_size, constant::Metrics::GAUGE);
      metrics_manager_->pushCommonMetricsToRemote(
          "work_queue_size", work_queue_size, constant::Metrics::GAUGE);
      metrics_manager_->pushCommonMetricsToRemote(
          "gc_count", KernelTraceManager::getInstance().getGcCount(),
          constant::Metrics::GAUGE);
      metrics_manager_->pushCommonMetricsToRemote(
          "data_loader_count",
