...
void MetricsManager::pushCommonMetricsToRemote(const std::string& metric_name,
                                               double value, int type) {
  auto it = common_metrics_.find(metric_name);
  if (it == common_metrics_.end()) {
    auto m = std::make_shared<metrics::CommonMetrics>(value, type, metric_name);
    client_stub_->requestRegisterPrometheus(
        true, metric_name,
        std::string(constant::Metrics::CommonMetrics::GAUGE_PREFIX),
        config::GlobalConfig::rank, config::GlobalConfig::local_rank, m->label);
    common_metrics_[metric_name] = m;
  }
  client_stub_->pushPrometheusCommonMetrics(
      false, metric_name, config::GlobalConfig::local_rank, value, type);
}
