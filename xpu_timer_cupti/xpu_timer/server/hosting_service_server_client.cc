...
void LocalPrometheusService::push(const BrpcMetrics* metrics) noexcept {
  if (metrics->has_kernel_metrics()) {
    ...
    // common metrics only has one value
    if (common_metrics.type() == constant::Metrics::COUNTER)
      gauges_->at(0)->Increment();
    else if (common_metrics.type() == constant::Metrics::GAUGE)
      gauges_->at(0)->Set(common_metrics.value());
...
void ClientStub::pushPrometheusCommonMetrics(bool sync, const std::string& name,
                                             int local_rank, double value,
                                             int type) {
  BrpcMetrics request;
  ...
  common_metrics->set_name(name);
  common_metrics->set_value(value);
  common_metrics->set_type(type);
