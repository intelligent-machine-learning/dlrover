 ...
 public:
  ...
  /*
        ...
        deregister metrics via the refs in the object.
                      ...
                      ┌─────────────────────┐       ┌──────────────┐
                      │XPU_TIMER_MM_KERNEL  ├──────►│prometheus::  │
        all_familys_  ├─────────────────────┤       │Family<Gauge> ├──────┐
                      │XPU_TIMER_NCCL_KERNEL│       ├──────────────┤      │
                      └─────────────────────┘       │QPS           │      │
                                                    ├──────────────┤      │
                                                    │AVG LATENCY   │      │
                                                    ├──────────────┤  create gauge
     ...
     ├──────────────────────────────────────┤       │Gauge[7]      │◄─────┘
     │xpu_timer_nccl_kernel_all_gather_ring │       ├──────────────┤
     └──────────────────────────────────────┘       │QPS           │
                                                    ├──────────────┤
                                                    │AVG LATENCY   │
                                                    ├──────────────┤
...
class ClientStub {
 public:
  ...
  void pushPrometheusCommonMetrics(bool sync, const std::string& name,
                                   int local_rank, double value, int type);
